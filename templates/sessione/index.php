...
<body>
    ...
    <div class="container my-4">
        ...
        <div class="row gap-4 justify-content-center mt-5">
            <?php $result = $conn->query("SELECT * FROM sessione_test WHERE classe='".$_SESSION['class']."'"); ?>
            <?php while ($sessione = $result->fetch_assoc()): ?>
            <?php
            $result_test = $conn->query("SELECT * FROM test WHERE id=".$sessione['id_test']);
            $test = $result_test->fetch_assoc();

            $result_docente = $conn->query("SELECT cognome FROM utente WHERE id=".$sessione['id_docente']);
                ...
                <div class="card h-100 position-relative">
                    ...
                    <div class="card-body position-static d-flex flex-column">
                        ...
                        <div class="d-flex flex-column mt-auto">
                            ...
                            <a href="view.php?id=<?= $sessione['id_test'] ?>" class="btn btn-primary stretched-link">Tenta test</a>
                        </div>
                    </div>
                </div>
            </div>
